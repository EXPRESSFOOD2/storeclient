    ...
    if (login.valid) {
      ...
      try {
        ...
        window.localStorage.setItem(
          "userData",
          JSON.stringify({ name, img, token, user_id })
        );
      } catch (error) {
        console.error(error);
      }
      dispatch(changeLoginStatus(true));
      //   createRoot(document.getElementById("alert")).render(
      //     <Alert
      //       title="Success"
...
export const getAllIngredients = () => {
  return async function (dispatch) {
    try {
      ...
      const ingredients = (await axios.get("/ingredients/get", { headers }))
        .data;
...
export const getIngredientById = (id) => {
  return async function (dispatch) {
    try {
      ...
      const ingredient = (
        await axios.get(`/ingredients/get/${id}`, { headers })
      ).data;
...
export const getReceta = () => {
  return async function (dispatch) {
    try {
      ...
      const receta = result.filter(
        (element) => !element.name?.includes(" OLD ")
      );
...
const sortBalanceByDate = (array) => {
  ...
  for (const element of array) {
    ...
    obj[date]
      ...
      : (obj[date] = {
          data: [element],
          amount: parseInt(element.totalAmountPerorder),
        });
  }
  return obj;
};
...
export const createRecipe = (recipe) => {
  return async () => {
    try {
      ...
      const newRecipe = (
        await axios.post("/recipes/create", recipe, { headers })
      ).data;
